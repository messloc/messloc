...
impl<T: Initer, const N: usize> DynArray<T, N> {
    pub fn create() -> Self {
        ...
        let pointer_slice = unsafe { core::ptr::slice_from_raw_parts_mut(pointers, N).as_mut().unwrap() };
        (0..N).for_each(|n| {
            let element = unsafe { OneWayMmapHeap.malloc(core::mem::size_of::<T>()) } as *mut MaybeUninit<T>;

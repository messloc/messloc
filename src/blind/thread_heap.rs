...
impl<R: Rng, H> ThreadHeap<R, H> {
    ...
    pub unsafe fn alloc(&mut self, layout: Layout) -> Result<NonNull<[u8]>, MiniHeapRequest> {
        ...
        if let Some(index) = SIZE_CLASSES
            .iter()
            .position(|&size_class| size_class as usize >= layout.size())
        {
    ...
    pub fn replace_mini_heap(
        &mut self,
        request: MiniHeapRequest,
        mini_heap: MiniHeap<H>,
    ) -> Option<MiniHeap<H>> {
...
impl<R, H> ThreadHeap<R, H> {
    pub unsafe fn drop_heaps<S: SpanAllocator<Handle = H>>(
        &mut self,
        span_alloc: &mut S,

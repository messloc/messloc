...
use super::{
    ...
    span::{Span, SpanAllocator},
    thread_heap::ThreadHeap,
    SpanVec, SIZE_CLASSES, SIZE_CLASS_COUNT,
};

type ThreadLocalLookup<R, S> = SpanVec<
    (
        usize,
        UnsafeCell<ThreadHeap<R, <S as SpanAllocator>::Handle>>,
    ),
    S,
>;
...
impl<R, S: SpanAllocator> Drop for GlobalHeap<R, S> {
    fn drop(&mut self) {
        for thread_local in self.thread_local.get_mut().as_slice_mut() {
            unsafe { (&mut *thread_local.1.get_mut()).drop_heaps(&mut self.span_alloc) }
                .map_err(|_| ())
                .unwrap();
        }
        ...
        for mini_heaps in &mut self.extra_mini_heaps {
            for mini_heap in mini_heaps.get_mut().as_slice_mut() {
                unsafe {
                    self.span_alloc
                        .deallocate_span(mini_heap.span_mut())
                        .map_err(|_| ())
                        .unwrap();
                }
            }
...
impl<R, S: SpanAllocator + Clone> GlobalHeap<R, S> {
    pub fn new(span_alloc: S, rng: R) -> Result<Self, S::AllocError> {
        let thread_local = unsafe { SpanVec::with_capacity_in(0, span_alloc.clone())? };

        let extra_mini_heaps = array_init::try_array_init(|_| {
            Ok(Mutex::new(SpanVec::with_capacity_in(
                0,
                span_alloc.clone(),
            )?))
        })?;
...
impl<R: Rng + SeedableRng, S: SpanAllocator> GlobalHeap<R, S> {
    pub unsafe fn alloc(&self, layout: Layout) -> Result<NonNull<[u8]>, S::AllocError> {
        ...
        if layout.size() > SIZE_CLASSES[SIZE_CLASS_COUNT - 1] as usize {
            ...
            let ptr = unsafe {
                NonNull::new_unchecked(slice_from_raw_parts_mut(
                    span.data_ptr().as_ptr(),
                    span.pages() as usize * self.span_alloc.page_size(),
                ))
            };
        ...
        match thread_heap.alloc(layout) {
            Ok(ptr) => Ok(ptr),
            Err(request) => {
                let pages = ((request.size_class() as usize * MAX_ALLOCATIONS_PER_SPAN)
                    / self.span_alloc.page_size())
                .min(u16::MAX as usize)
                .max(1);
                let mini_heap = MiniHeap::new(
                    self.span_alloc.allocate_span(pages.try_into().unwrap())?,
                    self.span_alloc.page_size(),
                    request.size_class(),
                );
                ...
                Ok(thread_heap
                    .alloc(layout)
                    .map_err(|_| ())
                    .expect("Mini heap was just replaced"))
            }
        }
    }
    ...
    pub unsafe fn dealloc(
        ...
        ptr: NonNull<[u8]>,
        layout: Layout,
    ) -> Result<(), S::DeallocError> {
        todo!();
    }

    fn create_thread_heap(
        &self,
        thread_id: usize,
        thread_local: RwLockUpgradableGuard<'_, ThreadLocalLookup<R, S>>,
    ) -> Result<*mut ThreadHeap<R, S::Handle>, S::AllocError> {
        let item = (
            thread_id,
            UnsafeCell::new(ThreadHeap::new(R::seed_from_u64({
